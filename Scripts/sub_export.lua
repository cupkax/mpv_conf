...
function export_selected_subtitles()
    ...
    while i < tracks_count do
        ...
        if track_type == "sub" and track_selected == "yes" then
            ...
            "/" .. mp.get_property("filename/no-ext") .. subtitles_ext

            mp.osd_message("Exporting selected subtitles")

            args = { ffmpeg_path, '-y', '-hide_banner', '-loglevel', 'error', '-i', video_file, "-map", string.format(
            "0:%d", track_index), subtitles_file }
